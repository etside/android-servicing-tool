...
class DeviceHeader(QFrame):
    ...
    def update(self, info: dict):
        if not info:
            ...
            return
        brand = info.get('brand', info.get('manufacturer', ''))
        ...
        # Format device name
        if platform == 'ios':
            name = f"Apple {model} · {mode.upper()}"
        else:
            name = f"{brand} {model}".strip() or "Unknown Device"
        
        self._name.setText(name)
        self._f["Serial"].setText(info.get("serial","—"))
        self._f["Mode"].setText(mode.upper())
        self._f["Android"].setText(str(info.get("android_version", info.get("version","—"))))
        ...
        # Color-code mode
        mode_colors = {
            'adb': CGREEN, 'fastboot': CORANGE, 'recovery': CORANGE,
            'qualcomm_edl': CRED, 'mediatek_brom': CRED, 'samsung_download': CRED,
            'charging': CMUTED, 'unauthorized': CORANGE,
            'ios_normal': CGREEN, 'ios_recovery': CORANGE, 'ios_dfu': CRED,
        }
        color = mode_colors.get(mode, CGREEN)
        self._pill.setText(f"● {mode.upper()}")
        self._pill.setStyleSheet(
            f"color:{color};background:rgba(0,0,0,0.25);border-radius:10px;"
            "padding:3px 10px;font-size:11px;font-weight:600;")
    ...
    def _refresh(self):
        ...
        if not BACKEND:
            self._apply({"model":"Demo Device","serial":"DEMO001","mode":"adb",
                         "android_version":"14","manufacturer":"Demo","brand":"Demo",
                         "chipset":"Unknown","platform":"android"})
            return
        ...
        def _detect():
            try:
                ...
                if not raw:
                    return {}
                info = dict(raw)
                info.setdefault("brand", info.get("manufacturer", ""))
                info.setdefault("manufacturer", info.get("brand", ""))
                info.setdefault("platform", "android")
                return info
            except Exception as e:
                return {"_error": str(e)}

        def _done(ok, result_str):
            # result_str is str(info dict) from Worker — re-detect properly
            pass

        # Run in background thread to avoid blocking UI
        self._thread, self._worker = run_async(
            self, lambda: _detect(), (), lambda ok, _: None
        )
        # Use a simpler approach: QTimer + direct thread
        import threading
        def _bg():
            info = _detect()
            # Marshal back to main thread via QTimer
            QTimer.singleShot(0, lambda: self._apply(info))
        threading.Thread(target=_bg, daemon=True).start()

    def _apply(self, info: dict):
        if info.get("_error"):
            self._log(f"Detection error: {info['_error']}")
            info = {}
        ...
        if info:
            ...
            self._sb_dev.setText(f"Device: {name}  [{serial}]  {mode.upper()}")
            self._log(f"✅ Device: {name}  serial={serial}  mode={mode}  platform={info.get('platform','?')}")
            if self._tray:
                try:
                    self._tray.showMessage("Device Connected", f"{name} ({mode.upper()})",
...
def main():
    """Entry point called by main.py and PyInstaller."""
    # Windows: enable DPI awareness
    if platform.system() == "Windows":
        try:
            from ctypes import windll
    ...
    import logging
    _log = logging.getLogger("etool-gui")
    if not request_access(_log):
        from PyQt6.QtWidgets import QMessageBox
        QMessageBox.critical(None, "Access Denied",
                             "Your access request was denied or timed out.\n"
